...
public class EquipmentService {
  ...
  EquipmentDTO getOneItem(Long id){
    return mapToDTO(equipmentRepository.findById(id).orElseThrow(()->new NoSuchElementException("item with id: "+id+" not exist")));
  }
  ...
  EquipmentDTO editEquipment(@PathVariable Long id, @RequestBody EquipmentDTO equipmentDTO)throws NoSuchElementException{
     Equipment equipment = equipmentRepository.findById(id).orElseThrow(()->new NoSuchElementException("item with id: "+id+" not exist"));
